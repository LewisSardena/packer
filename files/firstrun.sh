...
og_group_name = ${STACK_NAME}/ec2/${AUTOSCALING_GROUP}/var/log/ecs/ecs-agent
log_stream_name = {instance_id}
datetime_format = %Y-%m-%dT%H:%M:%SZ
EOF
...
if [[ -z ${ECS_CLUSTER}]]
  then
    echo "Skipping ECS agent check as ECS_CLUSTER variable is not define"
    exit 0
fi

# loop until ECS agent has registered to ECS cluster
echo "Checking ECS agent is join to ${ECS_CLUSTER}"

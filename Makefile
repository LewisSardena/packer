...
AWS_ROLE ?= arn:aws:iam::434442667797:role/admin
...
define assume_role
	$(eval AWS_SESSION = $(shell $(call get_assume_session,$(!))))
	$(eval export AWS_ACCESS_KEY_ID = $(shell $(call get_assume_credential,$(AWS_SESSION),AccessKeyID)))
	$(eval export AWS_SECREˇ_ACCESS_KEY = $(shell $(call get_assume_credential,$(AWS_SESSION),SecretAccessKey)))
